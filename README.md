...
1. 아맛다는 첫 앱스토어 출시 프로젝트로 기획과 디자인, 개발을 진행하게 되었습니다.
개발 공수를 계획하고 프로젝트를 진행하면서 기획과 디자인의 변경을 여러번 겪게 되었습니다.
이 과정을 통해 변경 사항에 대한 불필요한 수정 최소화와 공수를 지키지 못했을 경우의 대처를 배우게 되었습니다.

### **Problem • Try** 

1. 출시 기한에 맞춰 프로젝트 완성을 하려다보니 중복적인 코드들을 작성하게 되었습니다. 
중복적인 코드로 인해, 코드의 가독성이 떨어지고 유지보수 측면에서 비효율적이라 생각이 들었습니다.
모듈화 작업을 통해 가독성과 유지보수 측면을 개선할 것입니다.

2. 각 역할에 맞게 분류하지 못한 폴더링을 진행하였습니다.
협업 시 팀원이 알아보기 쉽도록 각 역할에 맞게 세분화한 폴더링으로 개선할 것입니다.

3. 해당 프로젝트는 MVC 구조로 설계 하였는데 각 구조에 맞지 않는 로직을 구성한 부분들이 있었습니다. 
역할에 맞지 않는 불필요한 로직을 제거하고 구조에 맞게 로직을 구성하여 가독성과 유지보수적인 측면을 개선할 것입니다.
또한, MVC 패턴은 Controller의 역할이 비대해지는 것 같기에 다음에는 비즈니스 로직을 따로 분리할 수 있도록 MVVM 패턴을 공부하고 적용시켜봐야겠습니다.
